...
> 功能范围见 `docs/DesignLine.md`，服务端代码结构见 `docs/Structrue.md`，后续开发蓝图见 `docs/Blueprint.md`。
> 附录「实现状态」记录协议与当前代码的对应关系，代码演进时可能先于本文档更新。
...
| ---- | --------------------------------------------------------------------------------------------------------------------- |
| 协议 | TCP 长连接                                                                                                            |
| 分帧 | 多个 TLV 首尾相接，无额外分隔符；接收方先读满 4 字节头部，再按 `Length` 读满 Value                                    |
| 加密 | 未来启用 TLSv1.3；启用后字节流格式不变，TLS 只负责加密传输                                                            |
| 版本 | 头部不含协议版本号；版本演进通过新增 Tag 完成，已分配 Tag 的语义不得改变；破坏性变更必须通过预留的协商类 Tag 显式协商 |

### 1.2 字节序

- 所有多字节整数均为**大端序（network byte order）**。
- 实现侧：读入后用 `be16toh` / `be32toh` / `be64toh` 转本机序（当前代码用 `std::byteswap` 等价实现），写出前用 `htobe*`。
...
| Message / OnlineUser 等 | 变长           | 由固定字段按顺序拼接的复合结构，见 §6 |
...
- 字段长度约束：用户名 1 ~ 32 字节；密码 6 ~ 64 字节；内容长度见 §6.3。
...
- 所有 `Req` 请求 Value 的前 4 字节都是 `request_id`。
- 所有 `Resp` 响应 Value 的前 4 字节原样回传 `request_id`；`Push` 推送不包含 `request_id`。
...
- 单个 TLV 的 Value 最大 65535 字节（`Length` 字段上限）。
...
代码中的枚举定义位于 `bechat/tlv/message_tag.h`（`MessageTag::Req::Type` / `Resp::Type` / `Push::Type`）。
...
代码中的枚举定义位于 `bechat/tlv/status_code.h`（`StatusCode::Type`）。状态码为 `u16` 大端，出现在各响应的 `status` 字段。

| 数值   | 常量                                | 含义           | 使用场景                                                            |
| ------ | ----------------------------------- | -------------- | ------------------------------------------------------------------- |
| 0x0000 | `StatusCode::Ok`                    | 成功           | 请求处理成功                                                        |
| 0x0001 | `StatusCode::NotLoggedIn`           | 未登录         | 未登录就请求受保护接口                                              |
| 0x0002 | `StatusCode::MalformedPacket`       | 协议错误       | 未知/错误方向 Tag、Value 长度不匹配、String 越界、超过 max_payload  |
| 0x0003 | `StatusCode::InvalidParameter`      | 参数错误       | 用户名/密码/limit/content 不符合约束、非法 UTF-8、`request_id` 为 0 |
...
每个 Value 按字段顺序拼接。下文各表不再重复的通用规则：

- 所有 `Req` 请求 Value 前 4 字节为 `request_id (u32)`。
- 所有 `Resp` 响应 Value 前 4 字节为 `request_id (u32)`，服务端原样回传；随后紧跟 `status (u16)`。`Resp::Error` 例外，见 §6.8。
- `Push` 推送 Value 不包含 `request_id`，也没有 `status`。
...
1. 当前连接绑定该用户名；
2. 服务端先给当前连接返回 `Resp::Login`，其中 `status = Ok`；
3. 再向所有在线连接广播 `Push::UserJoined`（包含新登录用户，客户端可自行忽略自己）。
...
1. 消息写入服务端内存中的聊天室历史；
2. 服务端先给发送者返回 `Resp::SendMessage`，其中 `status = Ok`；
3. 再向所有在线连接广播 `Push::NewMessage`（包含发送者自己）。
...
1. **解析失败**：客户端发送 `Resp` / `Push` 区间 Tag、未知 Tag、Value 无法解析、`Length < 4`、`Length` 超过服务端上限。
...
4. **参数错误**：字段语义不合法（如用户名过短、`request_id` 为 0、limit 超范围、空消息）。返回对应响应 Tag，回传 `request_id`，`status = InvalidParameter`。
...
| 协议内容                                 | 代码位置                                                                    | 状态                                                                                               |
| ---------------------------------------- | --------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------- |
| TLV 线格式（大端 Tag / Length / Value）  | `bechat/tlv/tlv_message.{h,cpp}`、`bechat/core/session.cpp`                 | 已实现                                                                                             |
| 双 strand 读/写流水线                    | `bechat/core/session.cpp`                                                   | 已实现                                                                                             |
| Value 字段级编解码（u16/u32/u64/String） | `bechat/tlv/tlv_value_reader.h`、`tlv_value_writer.h`                       | 已实现                                                                                             |
| 响应/错误/推送构造                       | `bechat/tlv/tlv_codec.{h,cpp}`（`MakeResponse` / `MakeError` / `MakePush`） | 已实现                                                                                             |
| `request_id` 提取与回传                  | `TlvCodec::PeekRequestId`（不足 4 字节返回 `nullopt`，此时回传 0）          | 已实现                                                                                             |
| Tag 校验与分发                           | `bechat/proto/dispatcher.cpp`                                               | 部分：非法 Tag / 缺 request_id 返回 `Resp::Error`；合法 Tag 暂回固定测试响应，尚未接入业务 handler |
| 响应 Tag 映射                            | `MessageTag::ReqTag2Resp`（`bechat/tlv/message_tag.h`）                     | 已实现                                                                                             |
| `max_payload` 校验（§2）                 | -                                                                           | 未实现                                                                                             |
| `request_id == 0` 参数校验（§1.5）       | -                                                                           | 未实现                                                                                             |
| 登录状态、用户表、在线表、历史存储       | -                                                                           | 未实现                                                                                             |
| Push 广播（§6.6 / §6.7）                 | `ServerContext::Broadcast`                                                  | 空实现（[TODO]）                                                                                   |
